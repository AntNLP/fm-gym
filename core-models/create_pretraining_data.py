...
def write_instance_to_example_files(
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        masked_positions = list(instance.masked_lm_positions)
        masked_tokens = list(instance.masked_lm_labels)

        assert len(masked_positions) == len(masked_tokens)

        next_sentence_label = 1 if instance.is_random_next else 0

        features = {}
        features["tokens"] = tokens
        features["masked_positions"] = masked_positions
        features["masked_tokens"] = masked_tokens
        features["next_sentence_label"] = next_sentence_label

        writers[writer_index].write(json.dumps(features, ensure_ascii=False) + '\n')
